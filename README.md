...
```javascript
this - это всегда ссылка на текущий контекст вызова
контекст (в рамках вызова функции) это тот объект который вызовет эту функцию
window - это и есть мир в javascript (любая функция вместо него, будет вызываться в рамках глобального контекста)
() при вызове метода - это вызвать сейчас, не написав кобки - значит просто взять ссылку на метод
toggle - это добавить класс если его нет, если есть - то убрать (переключатель)

Замыкание - объявление одной функции внутри другой (притом видит все переменные родительского метода)
...
#### CallBack
```
инструкция обратного вызова, когда вы не знаете когда вызывать функцию
прописать следующие действия
